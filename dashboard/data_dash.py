...
def wordCloud():
    #df = loadData()
    df = pd.read_csv('csvfile_global_twitter_processed.csv')
    cleanText = ''
...
def stBarChart():
    #df = loadData()
    df = pd.read_csv('csvfile_global_twitter_processed.csv')
    dfCount = pd.DataFrame({'Tweet_count': df.groupby(['original_author'])['clean'].count()}).reset_index()
...
def langPie():
    #df = loadData()
    df = pd.read_csv('csvfile_global_twitter_processed.csv')
    dfLangCount = pd.DataFrame({'Tweet_count': df.groupby(['lang'])['clean'].count()}).reset_index()
